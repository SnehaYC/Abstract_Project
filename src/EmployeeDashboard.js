...
function EmployeeDashboard() {
  ...
  const submitTravelRequest = (e) => {
    e.preventDefault();
    // Add the new request to the travel requests list
    setTravelRequests([...travelRequests, { ...newRequest, id: Date.now() }]);
    ...
    <div className="employee-dashboard">
      ...
      <ul>
        {travelRequests.map((request) => (
          <li key={request.id}>
            {request.employeeName} - {request.projectName} - {request.travelReason} - {request.bookingType}
          </li>
        ))}
      </ul>
    </div>
